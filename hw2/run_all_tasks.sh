...
echo "=========================================="
echo "Автоматический запуск всех задач hw2"
echo "=========================================="
echo ""

# Массив задач (в порядке выполнения)
tasks=("task1" "task2a" "task2b" "task2c" "task3" "task4" "task5" "task6")

# Маппинг номеров на имена задач для удобства
declare -A task_map=(
    ["1"]="task1"
    ["2a"]="task2a"
    ["2b"]="task2b"
...
run_task() {
    ...
    if [ "$NO_COMPILE" != "true" ]; then
        ...
        # Для task2b может быть make all
        if [ "$task" == "task2b" ]; then
    ...
    if [ -n "$job_id" ] && [[ "$job_id" =~ ^[0-9]+$ ]]; then
        ...
        echo "$job_id" > /tmp/hw2_${task}_jobid.txt
    else
...
if [ "$1" == "--help" ] || [ "$1" == "-h" ]; then
    ...
    echo "  1    - task1  (Global Memory)"
    echo "  2a   - task2a (Unified Memory)"
    echo "  2b   - task2b (CUDA Streams)"
    echo "  2c   - task2c (Shared Memory)"
    echo "  3    - task3  (Bank Conflicts Optimization)"
    echo "  4    - task4  (cuBLAS)"
    echo "  5    - task5  (OpenMP GPU)"
    echo "  6    - task6  (Nsight Profiling)"
    echo ""
    echo "Примеры:"
    echo "  $0                  Запустить все задачи"
    echo "  $0 1 2a 3          Запустить задачи 1, 2a, 3"
    echo "  $0 task1 task2b    Запустить задачи по имени"
...
if [ "$1" == "--status" ]; then
    ...
    for task in "${tasks[@]}"; do
        job_file="/tmp/hw2_${task}_jobid.txt"
        if [ -f "$job_file" ]; then
            job_id=$(cat "$job_file")
...
if [ "$1" == "--compile-only" ]; then
    ...
    for task in "${tasks[@]}"; do
        ...
        if [ -d "$task_dir" ]; then
            ...
            cd "$task_dir"
            make clean > /dev/null 2>&1
            # Для task2b используется make all
            if [ "$task" == "task2b" ]; then
                if make all; then
...
if [ $# -eq 0 ]; then
    ...
    tasks_to_run=()
    for arg in "$@"; do
        # Проверяем, является ли аргумент полным именем задачи
        if [[ " ${tasks[@]} " =~ " ${arg} " ]]; then
            tasks_to_run+=("$arg")
        # Проверяем, является ли аргумент номером задачи
        elif [[ -n "${task_map[$arg]}" ]]; then
            tasks_to_run+=("${task_map[$arg]}")
        else
            echo -e "${RED}Ошибка: неверный аргумент '$arg'${NC}"
            echo "Используйте номера задач (1, 2a, 2b, 2c, 3, 4, 5, 6) или полные имена (task1, task2a, ...)"
...
if [ ${#job_ids[@]} -gt 0 ]; then
    ...
    echo "    tail -f ${tasks_to_run[0]}/output_*.txt"
    echo ""
    echo "  Отменить все задачи:"
    echo "    scancel ${job_ids[*]}"
    echo ""
    echo "  После завершения всех задач, создать графики:"
