...
int main(int argc, char* argv[]) {
    ...
    double h;                        // шаг по пространству (будет вычислен из N)
    const double l = 1.0;           // длина стержня
    const double u0 = 1.0;          // начальная температура
    const double T = 1e-4;          // конечное время (уменьшено для ускорения)
    
    // Размер сетки из аргументов командной строки
    int N = 10000;
    if (argc > 1) {
        N = std::atoi(argv[1]);
    }
    ...
    // Вычисление шага по времени из условия устойчивости
    double tau = 0.9 * h * h / k;  // 0.9 для запаса устойчивости
    int num_time_steps = static_cast<int>(T / tau);
    if (num_time_steps == 0) num_time_steps = 1;
    
    if (rank == 0) {
        std::cout << "=== Task 2: Анализ масштабируемости ===" << std::endl;
    ...
    // Инициализация начального распределения (только на root)
    ...
    if (rank == 0) {
        ...
        std::ofstream csv("scalability_results.csv", std::ios::app);
        csv << std::fixed << std::setprecision(6);
        csv << size << "," << N << "," << min_time << "," << max_time << "," << avg_time << std::endl;
        csv.close();
        
        std::cout << "Результаты добавлены в scalability_results.csv" << std::endl;
    }
